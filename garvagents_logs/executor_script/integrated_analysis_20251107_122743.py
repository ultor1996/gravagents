...
for det in detectors:
    ...
    try:
        # Remove deprecated 'cache' argument as per GWpy API change
        strain = TimeSeries.fetch_open_data(det, gps_start, gps_end)
...
for det in detectors:
    ...
    try:
        ...
        # GWpy attaches colorbars to the figure; access via fig.colorbars
        if hasattr(fig, 'colorbars') and fig.colorbars:
            cbar = fig.colorbars[0]
            cbar.set_label('Normalized energy')
        else:
            print(f"    !! Warning: No colorbar found for {det} plot.")
        # Save the plot
