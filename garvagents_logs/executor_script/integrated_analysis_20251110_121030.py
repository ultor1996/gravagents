...
for det in detectors:
    ...
    try:
        ts = GwpyTimeSeries.fetch_open_data(det, start_time, end_time)
...
for det in strain_data:
    ...
    data_duration = n_samples_det / fs

    # --- CRITICAL FIX: seg_len must be <= half data duration ---
    # Use Welch segment length as the largest power of 2 <= half the data length, but at least 32
    max_seglen = n_samples_det // 2
    # Find largest power of 2 <= max_seglen
    seglen_samples = 2 ** int(np.floor(np.log2(max_seglen))) if max_seglen >= 32 else max(32, max_seglen)
    if seglen_samples > max_seglen:
        seglen_samples = max_seglen
    if seglen_samples < 32:
        seglen_samples = 32
    if seglen_samples > n_samples_det:
        seglen_samples = n_samples_det // 2
    if seglen_samples < 32:
        print(f"  WARNING: Data for {det} is very short; PSD estimation may be poor.")
    ...
    try:
        ...
        # Ensure PSD is the correct length for later interpolation
        psds[det] = psd
...
for (m1, m2), temp_np in templates.items():
    ...
    for det in ['H1', 'L1']:
        try:
            ...
            psd = psds[det]
            # Interpolate PSD to correct length and delta_f
