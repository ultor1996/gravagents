...
for det in detectors:
    ...
    try:
        # FIX: Remove deprecated 'cache' argument per database guidance
        ts = TimeSeries.get(channels[det], gps_start, gps_end)
        strain_data[det] = ts
...
if pycbc_timeseries[ref_det] is None:
    # Robust error handling: print detailed message and exit gracefully
    print(f"ERROR: Reference detector '{ref_det}' data missing after loading and filtering. Cannot proceed with template generation.")
    print("Check previous error messages for details on why data loading or filtering failed.")
