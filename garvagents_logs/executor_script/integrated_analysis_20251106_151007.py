...
def estimate_psd(strain_bp, sample_rate, label):
    ...
    n_samples = len(data)
    # According to PyCBC docs and database: seg_len <= n_samples // 2, and >= 2
    max_seglen = n_samples // 2
    # Start with 2s, but adjust if needed
    seglen = int(min(2.0 * sample_rate, max_seglen))
    if seglen < 2:
        seglen = 2
    # Ensure seglen is a power of 2 for FFT efficiency, but not required
    print(f"{label}: Using segment length {seglen} samples ({seglen/sample_rate:.2f} s) for PSD estimation.")
    try:
        psd = welch(
            data,
            seg_len=seglen,
        ...
        # Ensure PSD length matches data length (resize is in-place!)
        if len(psd) != n_samples:
            psd.resize(n_samples)
